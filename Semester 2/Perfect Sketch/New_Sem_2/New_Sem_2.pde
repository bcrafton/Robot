...
int pingPin=53;
int x, y;
boolean follow_line=false, salt_drop_yes=false, salt_drop_no=false;
boolean white=false, black=false;
boolean express_mode=false, custom_mode=false;
int y1 = A0;
int x2 = A1;
int y2 = A2;
int x1 = A3;
...
void lcd_commands()
{
  lcd.println("Express Mode=1");
  lcd.setCursor(0,1);
  lcd.println("Custom Mode=2");
  do
  {
    touch_screen();
    if (place==1)
  ...
  while(express_mode==false && custom_mode==false);
  ...
  lcd.clear();
  lcd.println("White Line = 1");
  lcd.setCursor(0, 1);
  lcd.println("Black Line = 2");
  delay(500);
...
void stop_white()
{
  ...
  for (i = 0; i < NUM_SENSORS; i++)
  {
    sensorValues[i]=(sensorValues[i])/101;
    if (sensorValues[i]>min_val)
    {
      sensorValues[i] = 1;
      stoptotal = stoptotal + sensorValues[i];
    }
    if (stoptotal == 0)
    {
      left.detach();
