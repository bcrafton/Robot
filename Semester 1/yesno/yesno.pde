...
int info, time;
Servo left, right;
int button_1=3, button_2=4, read_1, read_2, pingPin=53;
boolean follow_line=false;
boolean white=false, black=false, yes_ping=false, no_ping=false;
...
void lcd_commands()
{
  ...
  while (yes_ping==false && no_ping==false);
  lcd.clear();
}
...
void ping()
{
  ...
  if (mm<80 && mm>0) // find a different value for mm max because it will hit something 80 mm from the sensor. 
  {
    left.detach();
    right.detach();
    delay(4000);
